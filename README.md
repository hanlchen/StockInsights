...
- `computed_metrics.current_price` (used by movers/sectors) is a snapshot from whenever the batch job last ran; `current_price`/`volume` on the stock-detail page are always live (yfinance `fast_info`, a cheap call rarely affected by rate-limiting).

**Company profile + valuation fundamentals (sector, industry, business summary, book value, P/E, EPS, dividend yield, beta, etc.) intentionally prefer the DB over a live fetch.** These all come from yfinance's `.info` call, which is heavier and far more prone to Yahoo's rate-limiting than `fast_info`/price. Since this data is slow-changing and already refreshed daily by `scripts.refresh_cache`, `get_stock_metrics()` skips the live `.info` call entirely whenever the DB's `computed_metrics` row for that ticker is less than `FUNDAMENTALS_DB_FRESHNESS_HOURS` (30h) old, and reads these fields from the DB instead -- both to avoid needless latency and to avoid adding load to an endpoint that's already unreliable. The daily batch job itself always attempts a live fetch regardless (`prefer_db_fundamentals=False` -- otherwise it would just keep re-reading its own prior day's snapshot forever and never actually refresh). If `.info` is attempted (fresh-enough DB snapshot missing, or this is the batch job) and fails/is rate-limited/comes back empty, the response is flagged `fundamentals_unavailable: true`; the frontend (`StockMetricsCard`, `PriceHeader`) shows an explicit "API unable to fetch this right now" message for any field that's still null in that case, instead of silently rendering "—" the same way it would for a stock that genuinely has no dividend, no listed sector, etc.
